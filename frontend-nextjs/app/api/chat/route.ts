...
breaker.fallback(() => {
  return new Response(
    JSON.stringify({
      error: "Service Unavailable (Circuit Open). Please try again later.",
    }),
...
breaker.on("open", () =>
  console.warn("🔴 Circuit Breaker OPEN: Backend is failing"),
);
breaker.on("halfOpen", () =>
  console.log("🟡 Circuit Breaker HALF-OPEN: Testing backend"),
);
breaker.on("close", () =>
  console.log("🟢 Circuit Breaker CLOSED: Backend is healthy"),
);
  ...
  try {
    ...
    if (!backendUrl.includes("localhost")) {
      try {
        ...
        console.error(
          "[Proxy] Failed to get service-to-service ID token:",
          err,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `[Proxy] Backend responded: Status=${response.status} Duration=${duration}ms`,
      );
