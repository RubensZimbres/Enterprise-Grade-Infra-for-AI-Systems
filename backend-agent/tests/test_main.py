...
def test_chat_endpoint_success(client, mock_auth_user, mocker):
    # Mock the protected_graph_invoke to avoid hitting Vertex AI
    mocker.patch("main.protected_graph_invoke", return_value="Hello from Mock AI")
...
def test_chat_endpoint_internal_error(client, mock_auth_user, mocker):
    # Mock protected_graph_invoke to raise an exception
    mocker.patch("main.protected_graph_invoke", side_effect=Exception("Something went wrong"))
...
def test_stream_endpoint_success(client, mock_auth_user, mocker):
    # Mock protected_graph_stream to return an async generator
    async def mock_stream_generator(message, session_id):
        yield "Hello "
        yield "from "
        yield "Stream"

    mocker.patch("main.protected_graph_stream", side_effect=mock_stream_generator)
...
def test_stream_endpoint_error(client, mock_auth_user, mocker):
    # Mock protected_graph_stream to raise an exception immediately
    mocker.patch("main.protected_graph_stream", side_effect=Exception("Stream failed"))
